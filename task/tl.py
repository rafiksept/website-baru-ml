...
def count_vehicles_and_annotate(street, source_video_path, day_period, date, day_period2time=day_period2time, annotated_dir="annotated"):
    ...
    clip = VideoFileClip(annotated_path)
    hasil_akhir = os.path.join("static/result/" f"{id_prediction}.mp4")
    clip.write_videofile(hasil_akhir)
    try:
        os.remove(annotated_path)
        os.remove(source_video_path)
        logger.info(f"File {annotated_path} berhasil dihapus")
        logger.info(f"File {source_video_path} berhasil dihapus")
    except FileNotFoundError:
        logger.info(f"File {annotated_path} tidak ditemukan")
        logger.info(f"File {source_video_path} tidak ditemukan")
    except PermissionError:
        logger.info(f"Tidak memiliki izin untuk menghapus file {annotated_path}")
        logger.info(f"Tidak memiliki izin untuk menghapus file {source_video_path}")
    except Exception as e:
        logger.info(f"Terjadi kesalahan: {e}")
